...
function countdownFun(){
	intId2 = setInterval(function(){
		...
		if (countdownVar == 0) {
			...
			endAudio.play();
				if (score < highScore) {
					$('#newHighScore').html('<img src="/img/try-again.png" height="225">');
					var tryAgainAudio = new Audio('/audio/try-again.mp3');
					tryAgainAudio.play();
					$('#countdown').html('');
				} else if (score == highScore){
					$('#newHighScore').html('<img src="/img/tie.jpg" height="225">');
					var tieAudio = new Audio('/audio/tie.mp3');
					tieAudio.play();
					$('#countdown').html('');
				} else {
				$('#newHighScore').html('<img src="/img/new-high-scrore-text.jpg" height="225">');
